...
class _StudentFormPageState extends State<StudentFormPage> {
  ...
  void initState() {
    ...
    _yearController = TextEditingController(text: widget.student?.yearOfStudy ?? '');
    _firstModuleController = TextEditingController(text: widget.student?.firstModule ?? '');
    _firstLevelController = TextEditingController(text: widget.student?.firstModuleLevel ?? '');
    _secondModuleController = TextEditingController(text: widget.student?.secondModule ?? '');
    _secondLevelController = TextEditingController(text: widget.student?.secondModuleLevel ?? '');
  ...
  Future<bool> _uploadFile() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Upload failed: $e. Ensure "student-documents" bucket exists.')),
        );
      }
      return false;
    } finally {
  ...
  Future<void> _saveApplication() async {
    ...
    if (success && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(widget.student == null ? 'Application submitted!' : 'Application updated!'),
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildTextField(_fullNameController, 'Full Name', Icons.person, 'Enter full name'),
                  _buildTextField(_studentNumberController, 'Student Number', Icons.badge, 'Enter student number'),
                  _buildTextField(_yearController, 'Year Of Study', Icons.calendar_today, 'Enter year of study'),
                  ...
                  _buildTextField(_firstModuleController, 'Primary Module', Icons.book, 'Enter module name'),
                  _buildTextField(_firstLevelController, 'Academic Level (e.g. Level 6)', Icons.layers, 'Enter level'),
                  
                  const SizedBox(height: 16),
                  _buildTextField(_secondModuleController, 'Secondary Module (Optional)', Icons.book_outlined, null),
                  _buildTextField(_secondLevelController, 'Secondary Module Level', Icons.layers_outlined, null),

                  const Divider(height: 32),
                  
                  CheckboxListTile(
                    value: _meetsRequirements,
                    title: const Text('I confirm that I meet the minimum requirements for this position.'),
                  ...
                  ListTile(
                    ...
                    title: Text(_selectedFile != null 
                      ...
                      : (_documentUrl != null ? 'Current document attached' : 'Upload Supporting Document')),
